...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex justify-between items-center">
        ...
        <h2 class="text-xl font-bold text-slate-800">Inscritos: {{ $evento->titulo }}</h2>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-slate-100">
        <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <div>
                ...
                <div class="text-2xl font-bold text-indigo-600">{{ count($inscritos) }} / {{ $evento->limite_vagas }}</div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Data Inscrição</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-slate-200">
                    @foreach($inscritos as $inscrito)
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{{ $inscrito->nome }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $inscrito->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-400">{{ date('d/m/Y H:i', strtotime($inscrito->data_inscricao)) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
