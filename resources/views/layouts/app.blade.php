                    ...
                    <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
                        ...
                        @auth
                            @if(Auth::user()->perfil == 'organizador')
                                <a href="{{ route('dashboard.organizer') }}" class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent hover:border-white text-sm font-medium hover:text-indigo-100 transition duration-150 ease-in-out">
                                    Área do Organizador
                                </a>
                            @else
                                <a href="{{ route('dashboard.participant') }}" class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent hover:border-white text-sm font-medium hover:text-indigo-100 transition duration-150 ease-in-out">
                                    Minhas Inscrições
                                </a>
                            @endif
                        @endauth
        ...
        <div class="hidden sm:hidden" id="mobile-menu">
            ...
            <div class="pt-4 pb-4 border-t border-indigo-700">
                ...
                <div class="mt-3 space-y-1">
                    @auth
                        @if(Auth::user()->perfil == 'organizador')
                            <a href="{{ route('dashboard.organizer') }}" class="block px-4 py-2 text-base font-medium text-indigo-100 hover:text-white hover:bg-indigo-700">Meus Eventos</a>
                            <a href="{{ route('events.create') }}" class="block px-4 py-2 text-base font-medium text-indigo-100 hover:text-white hover:bg-indigo-700">Criar Evento</a>
                        @else
                            <a href="{{ route('dashboard.participant') }}" class="block px-4 py-2 text-base font-medium text-indigo-100 hover:text-white hover:bg-indigo-700">Minhas Inscrições</a>
                        @endif

                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="block w-full text-left px-4 py-2 text-base font-medium text-indigo-100 hover:text-white hover:bg-indigo-700">Sair</button>
                        </form>
                    @endauth
                </div>
            </div>
